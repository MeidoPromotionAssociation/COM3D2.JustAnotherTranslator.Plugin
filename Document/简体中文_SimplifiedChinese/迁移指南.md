...
- 你看到以下任意一项 → 使用的是 LBWtranslation/LBWmodifier
  - `LBWmodifier.dll`
  - `LBWmodifier-Transator D3V.dll` 或 `LBWmodifier-Transator D4V.dll` 或 `LBWmodifier-Transator D2.dll` 或 `LBWmodifier-Transator D3.dll`
...
- IMGUITranslationLoader 是专门用于翻译插件 IMGUI 窗口的插件，JAT 没有此功能，无需删除。
...
较新版本的 LBWmodifier 分为翻译器和修改器

带有 `LBWmodifier-Modifier` 字样的 dll 是修改器插件(例如 `LBWmodifier-Modifier D2.dll`)，可以保留

带有 `LBWmodifier-Transator` 字样的 dll 是翻译插件(例如 `LBWmodifier-Transator D2.dll`)，需要删除

## 清理

文件位置不固定，建议直接搜索文件名并删除 `LBWmodifier.dll` `LBWmodifier-Transator D3V.dll`
`LBWmodifier-Transator D4V.dll` `LBWmodifier-Transator D2.dll` `LBWmodifier-Transator D3.dll`
...
删除 `COM3D2\BepInEx\plugins\Translator D2` 文件夹，这是插件 dll 所在位置

删除 `COM3D2\BepInEx\plugins\Translator D3` 文件夹，这是插件 dll 所在位置

删除 `COM3D2\BepInEx\plugins\LBWmodifier-Transator D4V.dll` 文件，这是插件 dll 所在位置

删除 `COM3D2\BepInEx\plugins\Transator D2\LBWmodifier-Transator D2.dll` 文件，这是插件 dll 所在位置

删除 `COM3D2\BepInEx\plugins\Modifier D2\LBWmodifier-Transator D2.dll` 文件，这是插件 dll 所在位置

删除 `COM3D2\BepInEx\plugins\Transator D3\LBWmodifier-Transator D3.dll` 文件，这是插件 dll 所在位置

删除 `COM3D2\BepInEx\plugins\Modifier D3\LBWmodifier-Transator D3.dll` 文件，这是插件 dll 所在位置
