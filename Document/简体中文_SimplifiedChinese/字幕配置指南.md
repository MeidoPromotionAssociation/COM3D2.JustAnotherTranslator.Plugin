...
- `字体 (Font)`: 设置字幕所使用的字体。您需要输入系统中已安装字体的名称。通用选项内有打印系统已安装字体的选项。如果字体未找到，将使用默认的 Arial 字体。
...
JAT 支持为每个说话人设置不同的名字颜色和字幕颜色，每种字幕类型（Base/Yotogi/Adv/Lyric）可独立设置。

每个说话人的自定义字幕颜色默认为关闭，关闭时使用 BepInEx 全局配置中的颜色。

说话人名字颜色会在该角色首次显示字幕时由插件根据人名自动生成，不同的名字会获得不同的颜色。

相同名字得出的颜色始终相同，但由于运行时不同，当游戏更改 Unity 版本时可能会得到不一样的结果。

当插件生成颜色过后，会生成一个配置文件并持久化到 `COM3D2\BepInEx\JustAnotherTranslator\SubtitleColors.json`，然后您就可以自行更改说话者的名字颜色和字幕颜色。
...
```json
{
  "说话者名称1": {
    "SubtitleColors": {
      ...
      "Yotogi": { "SpeakerColor": "#RRGGBB", "Enabled": false, "..." : "..." },
      "Adv": { "SpeakerColor": "#RRGGBB", "Enabled": false, "..." : "..." },
      "Lyric": { "SpeakerColor": "#RRGGBB", "Enabled": false, "..." : "..." }
    }
  },
  "说话人名称2": {
    "SubtitleColors": {
      "Base": { "SpeakerColor": "#RRGGBB", "Enabled": false, "..." : "..." },
      "Yotogi": { "SpeakerColor": "#RRGGBB", "Enabled": false, "..." : "..." },
      "Adv": { "SpeakerColor": "#RRGGBB", "Enabled": false, "..." : "..." },
      "Lyric": { "SpeakerColor": "#RRGGBB", "Enabled": false, "..." : "..." }
    }
...
- `SpeakerColor`：说话人名字颜色，使用 `#RRGGBB` 十六进制颜色代码。始终生效，不受 `Enabled` 开关影响。每种字幕类型可设置不同的说话人名字颜色。
- `Enabled`：是否启用该字幕类型的自定义颜色（文本/背景/描边），默认为 `false`。关闭时使用 BepInEx 全局配置中的颜色。**不影响** `SpeakerColor`。
- `TextColor`：文本颜色，十六进制颜色代码。
- `TextOpacity`：文本不透明度（0-1）。
- `BackgroundColor`：背景颜色，十六进制颜色代码。
- `BackgroundOpacity`：背景不透明度（0-1）。
- `OutlineColor`：描边颜色，十六进制颜色代码。
- `OutlineOpacity`：描边不透明度（0-1）。

文档中的 `"..."` 只是代表省略，并不是真的要填这个
...
```json
{
  "一条 留依": {
    "SubtitleColors": {
      ...
      "Yotogi": { "SpeakerColor": "#FCD5DE", "Enabled": false, "..." : "..." },
      "Adv": { "SpeakerColor": "#FCD5DE", "Enabled": false, "..." : "..." },
      "Lyric": { "SpeakerColor": "#FCD5DE", "Enabled": false, "..." : "..." }
    }
  },
  "Kurita Kasumi": {
    "SubtitleColors": {
      "Base": { "SpeakerColor": "#C7CEC1", "Enabled": false, "..." : "..." },
      "Yotogi": { "SpeakerColor": "#C7CEC1", "Enabled": false, "..." : "..." },
      "Adv": { "SpeakerColor": "#C7CEC1", "Enabled": false, "..." : "..." },
      "Lyric": { "SpeakerColor": "#C7CEC1", "Enabled": false, "..." : "..." }
    }
  }
}
```
...
v0.1.3 及以下版本仅支持配置颜色，文件位于`COM3D2\BepInEx\JustAnotherTranslator\SpeakerColor.json`
...
v0.1.4 及以上版本不再使用 `SpeakerColor.json`。
