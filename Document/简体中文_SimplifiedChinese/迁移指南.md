...
把脚本放置到 `COM3D2\LBWtranslation` 文件夹下

创建 2 个文件夹，一个 output 一个 input

运行脚本

`python lyric_csv_format_convert.py ./input ./output`

然后把 `./output` 内的文件复制到 `COM3D2\BepInEx\JustAnotherTranslator\<你设置的语言>\Lyric\`

或者手动转换后，把转换的文件复制到 `COM3D2\BepInEx\JustAnotherTranslator\Lyric\`
...
把脚本放置到 `COM3D2\i18nEx\<你设置的语言>` 文件夹下

创建 2 个文件夹，一个 output 一个 input

在 `COM3D2\i18nEx\<你设置的语言>` 文件夹下运行脚本

`python ui_csv_format_convert.py ./input ./output`

然后把 `./output` 内的文件复制到 `COM3D2\BepInEx\JustAnotherTranslator\<你设置的语言>\UI`
