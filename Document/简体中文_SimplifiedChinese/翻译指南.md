...
JAT 支持替换 uGUI、NGUI 以及 JAT 自己字幕的字体，在游戏内切换 dump 选项中的 `PrintOSFont/打印系统字体`，可以将系统内已安装且可用的字体打印到控制台和日志。

游戏默认使用的是 `NotoSansCJKjp-DemiLight`，虽然是 CJK 字体，但是字形是日式的，其他地区用户可能不习惯，你可以将其替换为其他字体。

例如你可以下载 [NotoSansCJKsc-DemiLight.otf](https://github.com/notofonts/noto-cjk/blob/main/Sans/OTF/SimplifiedChinese/NotoSansCJKsc-DemiLight.otf) ，并设置字体为 `Noto Sans CJK SC`，这样就是简体中文字形了。

可以使用日志中显示的字体，Unity 5.6 一般只支持 .otf 和 .ttf 格式，**且必须右键字体，选择为所有用户安装**。
