...
After the plugin generates the colors, it creates a configuration file and persists it to `COM3D2\BepInEx\JustAnotherTranslator\SubtitleColors.json`. You can then manually change the speaker's name color and subtitle colors.
...
- `SpeakerColor`: The color used for the speaker's name in `#RRGGBB` hexadecimal format. This is always active regardless of the `Enabled` toggle. Each subtitle type can have a different speaker name color.
- `Enabled`: Whether to enable custom subtitle colors (text, background, outline) for this subtitle type. Default is `false`. When disabled, the global BepInEx configuration colors are used. Does **not** affect `SpeakerColor`.
- `TextColor`: Text color in hexadecimal.
- `TextOpacity`: Text opacity (0-1).
- `BackgroundColor`: Background color in hexadecimal.
- `BackgroundOpacity`: Background opacity (0-1).
- `OutlineColor`: Outline color in hexadecimal.
- `OutlineOpacity`: Outline opacity (0-1).

The `"..."` in the document simply represents an omission; it doesn't mean you actually need to fill it in.
...
Versions v0.1.3 and below only support color configuration. The file is located at `COM3D2\BepInEx\JustAnotherTranslator\SpeakerColor.json`
...
Versions v0.1.4 and above no longer use `SpeakerColor.json`.
