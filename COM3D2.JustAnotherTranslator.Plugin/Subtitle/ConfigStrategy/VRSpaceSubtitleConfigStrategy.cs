...
public class VRSpaceSubtitleConfigStrategy : BaseSubtitleConfigStrategy
{
    ...
    public override void CreateSubtitleUI(SubtitleComponent component)
    {
        ...
        component._vrSubtitleContainer = vrContainer;
        ...
        component._canvasScaler = canvasScaler;
        ...
        component._backgroundImage = bgImage;
        bgImage.color = new Color(0, 0, 0, 0.8f); // 默认黑色背景，80%透明度

        // 获取背景RectTransform
        var bgRect = bgObj.GetComponent<RectTransform>();
        component._backgroundRect = bgRect;
    ...
    public override void ApplyConfig(SubtitleComponent component)
    {
        ...
        try
        {
            ...
            if (component._vrHeadTransform != null && component._vrSubtitleContainer != null)
            {
                // 设置初始位置
                var headForward = component._vrHeadTransform.forward;
                var headUp = component._vrHeadTransform.up;
                var headRight = component._vrHeadTransform.right;

                // 应用偏移角度（水平和垂直）
                var verticalRotation = Quaternion.AngleAxis(component._config.VRSubtitleVerticalOffset, headRight);
                var horizontalRotation = Quaternion.AngleAxis(component._config.VRSubtitleHorizontalOffset, headUp);
                var offsetDirection = horizontalRotation * verticalRotation * headForward;

                // 计算最终位置（头部位置 + 偏移方向 * 距离）
                var targetPosition = component._vrHeadTransform.position +
                                     offsetDirection * component._config.VRSubtitleDistance;

                // 设置初始位置
                component._vrSubtitleContainer.position = targetPosition;

                // 设置初始朝向
                component._vrSubtitleContainer.rotation = Quaternion.LookRotation(
                    component._vrSubtitleContainer.position - component._vrHeadTransform.position);

                // 设置Canvas的尺寸
                component._canvas.GetComponent<RectTransform>().sizeDelta = new Vector2(
                    component._config.VRSubtitleWidth * 1000,
                    component._config.BackgroundHeight * 2);

                // 设置Canvas的缩放
                component._canvas.transform.localScale = Vector3.one * component._config.VRSubtitleScale * 0.001f;
            }
            else
            {
        ...
        float duration)
    {
        ...
        LogManager.Debug($"VR subtitle shown: {text}");
    }
